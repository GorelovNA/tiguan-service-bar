...
export class ProgressBarComponent extends BaseComponent implements OnInit {
    ...
    _jobs.forEach(j => {
      if (j.justOnce) {
        ...
        if (sameValueItem) {
          ...
          sameValueItem.fullCost = sameValueItem.jobs.reduce(
            (sum, curr) => (sum += curr.cost || 0),
            0
          );
        } else {
  ...
  ngOnInit(): void {
    this.currentValueCtrl.valueChanges
    ...
    while (value <= this.MAX_VALUE) {
      const text = this.isKmType
        ? value + ' т.км'
        : new Date(startDate.setMonth(startDate.getMonth() + step));
